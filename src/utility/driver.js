...
class Driver {
  ...
  */
  ...
  shouldSeeElementWithTextContent(element, text) {
    this.waitForAndReturnText(element, Constants.getMeduimWait()).should.contain(text);
  }
  ...
  waitForElementNotToExist(element) {
    browser.waitForExist(element, Constants.getLongWait()); // Wait for element to exist
    browser.waitForExist(element, Constants.getLongWait(), true); // Now wait for it to NOT exist (2 mins)
  }
  ...
  /**
   * This method is created to change the app from production to development mode.
   * @param inputField
   * @param element
   * @param submitButton
   */
  developmentEnvironmentLogin(inputField, element, submitButton) {
    browser.waitForExist(element);
    browser.waitForVisible(element);

    for (let i = 0; i < 10; i++) {
      browser.touchAction({
        action: "tap", x: 142, y: 226
      });
    }
    browser.pause(1000);
    browser.addValue(inputField, process.env.MOBILETOKEN);
    browser.touchAction(submitButton, "tap");
  }

  /**
   * Single method created to sign into the mobile application from the front page
   * @param userId
   * @param userPw
   * @param userSelector
   * @param pwSelector
   */
  enterLoginPW(userId, userPw, userSelector, pwSelector) {
    browser.waitForVisible(userSelector);
    browser.waitForEnabled(userSelector);
    browser.addValue(userSelector, userId);
    browser.waitForVisible(pwSelector);
    browser.waitForEnabled(pwSelector);
    browser.addValue(pwSelector, userPw);
  }
